...
namespace OaksMayFall
{
    ...
    public class ThirdPersonLocomotionController
    {
	    ...
	    public void GroundedCheck()
        {
            ...
            Owner.IsGrounded = Physics.CheckSphere(spherePosition, groundedRadius, groundLayers, QueryTriggerInteraction.Ignore);
        }
        ...
        {
	        if (Owner.IsGrounded && Owner.VerticalVelocity.y < 0.0f)
		        Owner.VerticalVelocity.y = -2f;
	        else if (Owner.VerticalVelocity.y < terminalVelocity)
		        Owner.VerticalVelocity.y += gravity * Time.deltaTime;
        }
        ...
        public void Move()
        {
	        if(Owner.IsFreezingMove)
				return;

	        Owner.HorizontalVelocity = Owner.IsSprinting ? GetSprintSpeed() : GetNormalSpeed();
	        Owner.CharacterCtr.Move((Owner.HorizontalVelocity + Owner.VerticalVelocity) * Time.deltaTime);
        }
        ...
        private Vector3 GetSprintSpeed()
        {
	        ...
	        // 否则冲刺速度趋向 0
				...
				return Vector3.SmoothDamp(Owner.HorizontalVelocity, Vector3.zero, ref sprintSmoothVelocity, sprintSmoothTime);
        }
        ...
        private Vector3 GetNormalSpeed()
        {
	        ...
	        return Vector3.SmoothDamp(Owner.HorizontalVelocity, targetVelocity, ref walkSmoothVelocity, walkSmoothTime);
        }
